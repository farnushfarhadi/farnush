		...
		if (B[nel] == 0) {
			...
			if (nel == 0) {
				...
				if (regulatorTypes[nel - 1] == RegulatorBindingData.TF) {
					 ...
					 * constraining in a certain direction CASE -1 0smaller1:
					 * ptr.nextptr[0].dmean < ptr.nextptr[1].dmeanchild -> coeff
					 * < 0 CASE +1 1smaller0: ptr.nextptr[0].dmean >
					 * ptr.nextptr[1].dmeanchild -> coeff > 0 CASE +2 bothUp: if
					 * miRNA downregulated -> coeff [-inf,+inf] CASE -2
					 * bothDown: if miRNA upregulated -> coeff [-inf,+inf]
					 */
