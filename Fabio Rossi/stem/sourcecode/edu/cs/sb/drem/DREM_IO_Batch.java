			...
			while ((szLine = br.readLine()) != null) {
				...
				if (st.hasMoreTokens()) {
					...
					if (!szvalue.equals("")) {
						...
						} else if (sztype.equalsIgnoreCase("Regulator_Types_Used_For_Activity_Scoring[TF,miRNA,Both]")) {
							if(szvalue.equalsIgnoreCase("TF")){
								checkStatusTF = true;
								...
								szError = "WARNING: '"
									+ szvalue
									+ "' is an invalid value for "
									+ "Regulator_Types_Used_For_Activity_Score it must be either 'TF'"
									+ ", 'miRNA' or 'Both'.\n";
							}
	...
	public void clusterscript(
		...
		if (nexceptions == 0) {
			...
			try {
				...
				// Must call the dynamic version first because
				// saveActivityScores
